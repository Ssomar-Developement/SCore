...
import com.ssomar.scoretestrecode.features.types.list.ListDetailedMaterialFeature;
    ...
    public boolean isValidMaterial(@NotNull Material material, Optional<String> statesStrOpt, Optional<Player> playerOpt, Event event) {
        ...
        if (!blocks.isValidMaterial(material, statesStrOpt) && !blocks.getValue().isEmpty()) {
            ...
            if (playerOpt.isPresent() && messageIfNotValid.getValue().isPresent()) {
                SendMessage.sendMessageNoPlch(playerOpt.get(), messageIfNotValid.getValue().get());
            }
            return false;
        }
