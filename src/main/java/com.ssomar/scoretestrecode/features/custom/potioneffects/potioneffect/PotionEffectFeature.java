...
public class PotionEffectFeature extends FeatureWithHisOwnEditor<PotionEffectFeature, PotionEffectFeature, PotionEffectFeatureEditor, PotionEffectFeatureEditorManager> {
    ...
    public PotionEffect getPotionEffect() {
        if(SCore.is1v12Less()) return new PotionEffect(type.getValue().get(), duration.getValue().get(), amplifier.getValue().get(), ambient.getValue(), particles.getValue());
        else return new PotionEffect(type.getValue().get(), duration.getValue().get(), amplifier.getValue().get(), ambient.getValue(), particles.getValue(), icon.getValue());
    }
