...
public class ListExecutableItemsFeature extends ListFeatureAbstract<String, ListExecutableItemsFeature> {
    ...
    public ListExecutableItemsFeature initItemParentEditor(GUI gui, int slot) {
        ...
        if(!SCore.hasExecutableItems) finalDescription[finalDescription.length - 2] = "&4&l● &c&lRequire ExecutableItems";
    ...
    public Optional<String> verifyMessageReceived(String message) {
        if(SCore.hasExecutableItems){
            ...
            else return Optional.of("&4&l[ERROR] &cThe message you entered is not a valid ExecutableItems ID ! (The id is word before the .yml) ");
        }
        return Optional.empty();
    }
