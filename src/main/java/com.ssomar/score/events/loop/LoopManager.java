...
import com.ssomar.testRecode.executableItems.activators.NewActivatorEIFeature;
import com.ssomar.testRecode.executableItems.activators.Option;
...
public class LoopManager {
            ...
            public void run() {
                ...
                if (!toActiv.isEmpty() || !checkEntityOnofEB.isEmpty()) {
                    ...
                    if (SCore.hasExecutableItems) {
                        try {
                            List<NewActivatorEIFeature> listEI = new ArrayList<>();

                            for (NewSActivator sActivator : toActiv) {
                                if (sActivator instanceof NewActivatorEIFeature) {
                                    listEI.add((NewActivatorEIFeature) sActivator);
                                }
                            }
                    ...
                    if (SCore.hasExecutableBlocks) {
                        ...
                        List<NewActivatorEBFeature> listEB = new ArrayList<>();
                        for (NewSActivator sActivator : checkEntityOnofEB) {
                            if (sActivator instanceof NewActivatorEBFeature) {
                                listEB.add((NewActivatorEBFeature) sActivator);
                            }
                        }
    ...
    public void runEntityOnEb(Location loc, ExecutableBlockPlaced eBP, List<NewActivatorEBFeature> listEB) {
    ...
    public void runLoopEB(ExecutableBlockPlaced eBP, List<NewActivatorEBFeature> listEB) {
        LoopEvent e = new LoopEvent();
        EventInfo eInfo = new EventInfo(e);
    ...
    public void resetLoopActivatorsEB() {
        ...
        for (NewSActivator sAct : loopActivators.keySet()) {
            if ((sAct instanceof NewActivatorEBFeature)) toRemove.add(sAct);
        }
    ...
    public void resetLoopActivatorsEI() {
        List<NewSActivator> toRemove = new ArrayList<>();
        for (NewSActivator sAct : loopActivators.keySet()) {
            if ((sAct instanceof NewActivatorEIFeature)) toRemove.add(sAct);
        }
        for (NewSActivator sAct : toRemove) loopActivators.remove(sAct);
    }
