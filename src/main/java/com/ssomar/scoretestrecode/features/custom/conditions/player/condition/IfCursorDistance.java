...
public class IfCursorDistance extends PlayerConditionFeature<NumberConditionFeature, IfCursorDistance> {
    ...
    public boolean verifCondition(Player player, Optional<Player> playerOpt, SendMessage messageSender, Event event) {
        if (hasCondition()) {
            ...
            if (!StringCalculation.calculation(getCondition().getValue().get(), player.getLocation().distance(block.getLocation()))) {
    ...
    @Override
    public IfCursorDistance getNewInstance() {
        return new IfCursorDistance(getParent());
    }
}
