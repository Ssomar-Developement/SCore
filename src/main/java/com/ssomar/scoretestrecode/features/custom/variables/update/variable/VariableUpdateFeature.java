...
public class VariableUpdateFeature extends FeatureWithHisOwnEditor<VariableUpdateFeature, VariableUpdateFeature, VariableUpdateFeatureEditor, VariableUpdateFeatureEditorManager> {
    ...
    public void reset() {
        ...
        this.type = new VariableUpdateTypeFeature(this, "type", Optional.of(VariableUpdateType.SET), "Type", new String[]{"&7&oThe type of update you want to do"}, Material.CLOCK, false);
        this.stringUpdate = new ColoredStringFeature(this, "update", Optional.of(""), "String Update", new String[]{"&7&oThe string update"}, GUI.WRITABLE_BOOK, false, false);
        this.doubleUpdate = new DoubleFeature(this, "update", Optional.of(0.0), "Double Update", new String[]{"&7&oThe number update"}, GUI.WRITABLE_BOOK, false);
    }

    public VariablesGroupFeature getVariables(){
        FeatureParentInterface parent = this.getParent();
        while(parent instanceof FeatureAbstract && ((FeatureAbstract)parent).getParent() != parent){
    ...
    @Override
    public VariableUpdateFeature clone() {
        VariableUpdateFeature eF = new VariableUpdateFeature(getParent(), id);
        eF.setVariableName(variableName.clone());
        eF.setType(type.clone());
        eF.setStringUpdate(stringUpdate.clone());
        eF.setDoubleUpdate(doubleUpdate.clone());
        return eF;
    }
