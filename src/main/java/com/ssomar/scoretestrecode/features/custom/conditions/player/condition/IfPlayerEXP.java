...
public class IfPlayerEXP extends PlayerConditionFeature<NumberConditionFeature, IfPlayerEXP> {
    ...
    public boolean verifCondition(Player player, Optional<Player> playerOpt, SendMessage messageSender, Event event) {
        if (hasCondition() && !StringCalculation.calculation(getCondition().getValue().get(), player.getTotalExperience())) {
    ...
    @Override
    public IfPlayerEXP getNewInstance() {
        return new IfPlayerEXP(getParent());
    }
}
