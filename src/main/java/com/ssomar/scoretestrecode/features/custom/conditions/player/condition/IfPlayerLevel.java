...
public class IfPlayerLevel extends PlayerConditionFeature<NumberConditionFeature, IfPlayerLevel> {
    ...
    public boolean verifCondition(Player player, Optional<Player> playerOpt, SendMessage messageSender, Event event) {
        if (hasCondition() && !StringCalculation.calculation(getCondition().getValue().get(), player.getLevel())) {
    ...
    @Override
    public IfPlayerLevel getNewInstance() {
        return new IfPlayerLevel(getParent());
    }
