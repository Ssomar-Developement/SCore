...
public class ListEnchantAndLevelFeature extends FeatureAbstract<Map<Enchantment, Integer>, ListEnchantAndLevelFeature> implements FeatureRequireSubTextEditorInEditor {
    ...
    public ListEnchantAndLevelFeature(FeatureParentInterface parent, String name, Map<Enchantment, Integer> defaultValue, String editorName, String[] editorDescription, Material editorMaterial, boolean requirePremium) {
    ...
    public Map<Enchantment, Integer> transformEnchants(List<String> enchantsConfig, List<String> errorList){
        ...
        for(String s : enchantsConfig){
            ...
            if (s.contains(":")) {
                ...
                if (error) errorList.add("&cERROR, Couldn't load the Enchant with level value of " + this.getName() + " from config, value: " + s+ " &7&o"+getParent().getParentInfo()+" &6>> correct form > ENCHANTMENT:LEVEL  example> POWER:1 !");
            }
        }
        return result;
    }
    ...
    @Override
    public ListEnchantAndLevelFeature clone() {
        ListEnchantAndLevelFeature clone = new ListEnchantAndLevelFeature(getParent(), this.getName(), getDefaultValue(), getEditorName(), getEditorDescription(), getEditorMaterial(), isRequirePremium());
    ...
    public Optional<String> verifyMessageReceived(String message) {
        ...
        if (s.contains(":")) {
            ...
            if(error) return Optional.of("&4&l[ERROR] &cThe message you entered &8(&7"+s+"&8)&c is not an enchant with level &6>> correct form > ENCHANTMENT:LEVEL  example> POWER:1 !");
        }
