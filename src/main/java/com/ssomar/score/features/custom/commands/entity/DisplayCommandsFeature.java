...
import com.ssomar.score.commands.runnable.entity.display.DisplayCommandManager;
import com.ssomar.score.commands.runnable.entity.display.DisplayRunCommandsBuilder;
...
@Getter
@Setter
public class DisplayCommandsFeature extends CommandsAbstractFeature<List<String>, DisplayCommandsFeature> implements FeatureRequireSubTextEditorInEditor {

    private List<String> value;

    public DisplayCommandsFeature(FeatureParentInterface parent, FeatureSettingsInterface featureSettings) {
    ...
    public List<String> load(SPlugin plugin, ConfigurationSection config, boolean isPremiumLoading) {
        ...
        value = DisplayCommandManager.getInstance().getCommandsVerified(plugin, config.getStringList(getName()), errors, getParent().getParentInfo());
        return errors;
    }
    ...
    public void runCommands(ActionInfo actionInfo, String objectName) {
        ...
        DisplayRunCommandsBuilder builder2 = new DisplayRunCommandsBuilder(commands, actionInfo);
    ...
    @Override
    public DisplayCommandsFeature initItemParentEditor(GUI gui, int slot) {
    ...
    @Override
    public DisplayCommandsFeature clone(FeatureParentInterface newParent) {
        DisplayCommandsFeature clone = new DisplayCommandsFeature(newParent, getFeatureSettings());
    ...
    public Optional<String> verifyMessageReceived(String message) {
        return DisplayCommandManager.getInstance().verifCommand(message);
    }
    ...
    public List<Suggestion> getSuggestions() {
        ...
        for (SCommand command : DisplayCommandManager.getInstance().getCommands()) {
    ...
    public void sendBeforeTextEditor(Player p, NewGUIManager manager) {
        ...
        TextComponent variables = new TextComponent(StringConverter.coloredString("&7➤ WIKI of Display commands: &8&l[CLICK HERE]"));
        ...
        EditorCreator editor = new EditorCreator(beforeMenu, (List<String>) manager.currentWriting.get(p), "Display Commands", false, true, true, true, true, true, false, "", commands);
