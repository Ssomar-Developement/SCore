...
public class DetailedSlotsEditor extends FeatureEditorInterface<DetailedSlots> {
    ...
    public void updateSlotOffHand(int i, boolean enable) {
        ...
        if (enable)
            createItem(offHandMat, 1, i, "&eSlot: offHand", true, false, "", "&a✎ Click here to change", TM.g(Text.EDITOR_CURRENTLY_NAME)+" &aEnable");
        else
            createItem(offHandMat, 1, i, "&eSlot: offHand", false, false, "", "&a✎ Click here to change", TM.g(Text.EDITOR_CURRENTLY_NAME)+" &cDisable");
    }
    ...
    public boolean addOrRemoveEnchant(ItemStack item) {
        if (this.getCurrently(item).contains("Enable")) {
            //SsomarDev.testMsg("enable to disable Item: " + item, true);
            ...
            //SsomarDev.testMsg("disable to enable Item: " + item, true);
    ...
    public void updateSlot(int i, int slot, boolean enable) {
        if (enable) {
            if (!SCore.is1v12Less())
                createItem(Material.GREEN_WOOL, 1, i, "&eSlot: " + slot, false, false, "", "&a✎ Click here to change", TM.g(Text.EDITOR_CURRENTLY_NAME)+" &aEnable");
            else
                createItem(Material.EMERALD, 1, i, "&eSlot: " + slot, false, false, "", "&a✎ Click here to change", TM.g(Text.EDITOR_CURRENTLY_NAME)+" &aEnable");
        } else {
            if (!SCore.is1v12Less())
                createItem(Material.RED_WOOL, 1, i, "&eSlot: " + slot, false, false, "", "&a✎ Click here to change", TM.g(Text.EDITOR_CURRENTLY_NAME)+" &cDisable");
            else
                createItem(Material.REDSTONE, 1, i, "&eSlot: " + slot, false, false, "", "&a✎ Click here to change", TM.g(Text.EDITOR_CURRENTLY_NAME)+" &cDisable");
        }
    }
