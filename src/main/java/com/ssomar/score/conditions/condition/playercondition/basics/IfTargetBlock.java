...
public class IfTargetBlock extends PlayerCondition<List<Material>> {


    public IfTargetBlock() {
        super(ConditionType.LIST_MATERIAL, "ifTargetBlock", "If target block", new String[]{}, new ArrayList<>(), " &cYou don't target the good type of block to active the activator: &6%activator% &cof this item!");
    }
    ...
    public boolean verifCondition(Player player, Optional<Player> playerOpt, SendMessage messageSender) {
        if (getCondition() != null && getCondition().size() > 0) {
            ...
            if (!getCondition().contains(block.getType())) {
                sendErrorMsg(playerOpt, messageSender);
                return false;
            }
        }
