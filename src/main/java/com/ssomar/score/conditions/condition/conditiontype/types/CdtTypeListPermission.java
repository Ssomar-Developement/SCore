...
public class CdtTypeListPermission extends AConditionType {
    @Override
    ...
    public void buildCondition(List<String> stringList, Condition condition) {
        ...
        List<String> permissionsPlaceholder = new ArrayList<>();

        for(String biomeStr : stringList) {
            if(biomeStr.startsWith("%")) {
                permissionsPlaceholder.add(biomeStr);
                continue;
            }
            permissions.add(biomeStr);

        }
        condition.setCondition(permissions);
        if(permissionsPlaceholder.size() > 0) condition.setPlaceHolderCondition(Optional.of(permissionsPlaceholder));
    }

    @Override
    public <T extends Condition> void saveIn(ConditionGUI gui, T condition) {
        buildCondition(gui.getConditionList(ConditionGUI.CONDITION, "NO PERMISSION DEFINED"), condition);
    }
    ...
    public void updateGUIContains(ConditionGUI gui, Object condition) {
        gui.updateConditionList(ConditionGUI.CONDITION, (List<String>) condition, "&6➤ &eNO PERMISSION DEFINED");
    }

    @Override
    public void updateGUINotContains(ConditionGUI gui) {
        gui.updateConditionList(ConditionGUI.CONDITION, new ArrayList<>(), "&6➤ &eNO PERMISSION DEFINED");
    }


    @Override
    public void clickGUI(ConditionGUIManager manager, Player player) {
        manager.getRequestWriting().put(player, ConditionType.LIST_PERMISSION.toString());
        if (!manager.getCurrentWriting().containsKey(player)) {
            manager.getCurrentWriting().put(player, manager.getCache().get(player).getConditionList(ConditionGUI.CONDITION, "NO PERMISSION DEFINED"));
        }
        ...
        this.showPermissionEditor(manager, player);
        manager.space(player);
    }
    ...
    public void addLine(ConditionGUIManager manager, Player player, String message) {
        ...
        if (!uncoloredMessage.isEmpty()) {
                ...
                player.sendMessage(StringConverter.coloredString("&a&l" + manager.getCache().get(player).getsPlugin().getNameDesign() + " &2&lEDITION &aYou have added new permission!"));
        } else error = true;

        if (error)
            player.sendMessage(StringConverter.coloredString("&c&l" + manager.getCache().get(player).getsPlugin().getNameDesign() + " &4&lERROR &cEnter a valid permission !"));
    ...
    public void showPermissionEditor(ConditionGUIManager manager, Player player) {
        List<String> beforeMenu = new ArrayList<>();
        beforeMenu.add("&7➤ " + manager.getCache().get(player).getCondition().getConfigName() + ":");

        HashMap<String, String> suggestions = new HashMap<>();

        EditorCreator editor = new EditorCreator(beforeMenu, manager.getCurrentWriting().get(player), "Permission", false, false, false, true, true, true, false, "", suggestions);
