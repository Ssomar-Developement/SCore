...
public class IfLightLevel extends PlayerCondition<String> {
    ...
    public boolean verifCondition(Player player, Optional<Player> playerOpt, SendMessage messageSender) {
        if (getCondition() != null && getCondition().length() > 0 && !StringCalculation.calculation(getCondition(), player.getEyeLocation().getBlock().getLightLevel())) {
