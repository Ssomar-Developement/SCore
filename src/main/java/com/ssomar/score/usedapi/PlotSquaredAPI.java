...
public class PlotSquaredAPI {
    ...
    public static boolean playerIsInHisPlot(@NotNull Player p, Location location) {
        ...
        if (plot != null) {
            if (plot.getMembers().contains(p.getUniqueId()) || plot.getTrusted().contains(p.getUniqueId()) || plot.getOwner().equals(p.getUniqueId())) return true;
        }
    ...
    public static boolean playerCanBreakIslandBlock(@NotNull UUID pUUID, @NotNull Location location) {
        ...
        Plot plot = Plot.getPlot(loc);
        if (plot != null) {
            if (!(plot.getMembers().contains(pUUID) || plot.getTrusted().contains(pUUID) || plot.getOwner().equals(pUUID))) return false;
        }
