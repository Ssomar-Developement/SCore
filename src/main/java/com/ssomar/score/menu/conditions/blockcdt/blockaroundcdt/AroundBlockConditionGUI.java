...
public class AroundBlockConditionGUI extends ConditionGUIAbstract{
	...
	public AroundBlockConditionGUI(SPlugin sPlugin, SObject sObject, SActivator sActivator, List<AroundBlockCondition> list, String detail) {
		super(TITLE, 6 * 9, sPlugin, sObject, sActivator, detail, new AroundBlockCondition("null"));
		
		newAroundBlockCondition = true;

		int id = 1;
		for (int i = 0; i < list.size(); i++) {
			for (AroundBlockCondition aBC : list) {
		...
		AroundBlockCondition aBC = (AroundBlockCondition)this.getConditions();
		aBC.setId(idStr);
		this.setConditions(aBC);
		this.loadTheGUI();
	}

	public AroundBlockConditionGUI(SPlugin sPlugin, SObject sObject, SActivator sActivator, AroundBlockCondition aBC, String detail) {
		super("&8&l"+sPlugin.getShortName()+" "+TITLE, 6 * 9, sPlugin, sObject, sActivator, detail, aBC);
	}
