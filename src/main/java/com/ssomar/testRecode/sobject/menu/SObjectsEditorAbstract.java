...
import com.ssomar.testRecode.sobject.NewSObjectManager;
	...
	public void load() {
		...
		for (String str : listFiles) {
			if((index-1) * SOBJECT_PER_PAGE <= total && total < index * SOBJECT_PER_PAGE) {
				...
				if (fileEntry.isDirectory()) createItem(Material.CHEST, 				1 , i, 	"&2&l✦ FOLDER: &a"+str, 	false, false, "", "&7(click to open)");
				else {
					if(!fileName.contains(".yml")) createItem(Material.BARRIER, 				1 , i, 	"&4&l✦ INVALID FILE: &c"+str, 	false, false, "", "&4(shift + left click to delete)");
		...
		if(sPlugin.isLotOfWork()) createItem(PURPLE, 						1 , 43, 	"&5&l✚ &dDefault Premium "+sPlugin.getObjectName(), 	false, false);
