...
public class ExecutableItemFeature extends FeatureAbstract<Optional<ExecutableItemInterface>, ExecutableItemFeature> implements FeatureRequireOnlyClicksInEditor {
    ...
    public Optional<ExecutableItemInterface> getValue() {
        if(value.isPresent() && SCore.hasExecutableItems && ExecutableItemsAPI.getExecutableItemsManager().isValidID(value.get())) return ExecutableItemsAPI.getExecutableItemsManager().getExecutableItem(value.get());
    ...
    public ExecutableItemFeature initItemParentEditor(GUI gui, int slot) {
        ...
        if (value.isPresent() && SCore.hasExecutableItems && ExecutableItemsAPI.getExecutableItemsManager().isValidID(value.get())){
            item = ExecutableItemsAPI.getExecutableItemsManager().getExecutableItem(value.get()).get().buildItem(1, Optional.empty());
        }
    ...
    public void updateItemParentEditor(GUI gui) {
        if(SCore.hasExecutableItems && value.isPresent() && ExecutableItemsAPI.getExecutableItemsManager().isValidID(value.get())) {
            updateExecutableItem(value.get(), gui);
        }
        else updateExecutableItem(ExecutableItemsAPI.getExecutableItemsManager().getExecutableItemIdsList().get(0), gui);
    }

    @Override
    public void extractInfoFromParentEditor(NewGUIManager manager, Player player) {
        this.value = getExecutableItem( (GUI) manager.getCache().get(player));
    }

    @Override
    public ExecutableItemFeature clone() {
        ExecutableItemFeature clone = new ExecutableItemFeature(getParent(), this.getName(), getEditorName(), getEditorDescription(), getEditorMaterial(), requirePremium());
        clone.value = value;
        return clone;
    }
